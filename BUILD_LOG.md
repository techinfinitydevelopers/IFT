...
## 2026-08-31 — School dashboard: "Complete Your Profile" as a floating alert, not a hard blocker
- User wanted the mandatory full-screen profile-completion form (shown on first login while `School.status == 'pending'`) removed as a blocker. Requirement: pending schools should see the FULL dashboard immediately, but the other 8 dashboard pages (Teams/Students/Submissions/Results/Reports/Notifications/Announcements/Hall of Fame) must STAY locked until the profile form is filled — plus 3 more flagged in a follow-up: Marketing Collaterals, Learning Resources, IFTx Highlights.
- `students/views.py:school_dashboard` — removed the early-return that rendered a form-only template for pending schools. Now computes the full dashboard context for BOTH pending and active schools (`is_pending` flag passed through); only a truly non-active/non-pending status (admin-deactivated) still blocks + logs out.
- `templates/students/school_dashboard.html` — the old `{% if is_pending %}...{% else %}...{% endif %}` split (pending-only-form vs full-dashboard) is gone. The profile form now lives in a hidden modal (`#completeProfileModal`), opened by a fixed bottom-right FAB (`#completeProfileFab`, red, CSS `completeProfilePulse` pulse animation) shown only `{% if is_pending %}`. Chart json_script tags (grade/funnel/team_pie) unconditional now since the view always computes them. Existing AJAX submit handler (already did `window.location.reload()` on success) needed no changes — reload naturally drops the FAB once status flips to 'active'.
- No DB/migration changes — `is_pending` still derived from `School.status`, no new field.
- Verified via Django test client (`ALLOWED_HOSTS=['testserver']` override — local runserver drops `testserver`, see earlier gotcha): pending school → full dashboard + FAB + modal, but `/school/students/` and `/school/reports/` still redirect to dashboard; form submit flips `status` to `active`; active school unaffected (no FAB, direct 200s); students (no `school_profile`) unaffected.
- Follow-up: gated 3 more school pages behind the same `status != 'active'` check — `students/views.py:school_learning_resources`, `school_digital_resources`, and `highlights/views.py:my_highlights` + `upload_highlight` + `highlight_detail` (guarded with `if school and school.status != 'active'` since `my_highlights` is a SHARED view also used by students, who have no `school_profile`). Verified: pending school redirected on all 3; active school and a student account both unaffected.
- Local test accounts: `test_school_coord` (School id=3, `Test@1234`) reset to `pending` for manual browser testing of the new FAB/modal flow.
- Follow-up UX pass (user asked how to make the modal more noticeable): FAB changed from icon-only circle to a labeled red pill ("Complete Profile" + icon), and the modal now auto-opens once per browser session on first dashboard load (`sessionStorage` key `completeProfileAutoShown`, 800ms delay) — after that first auto-open, the FAB is the only reminder.
- Committed `<pending>`, pushed.
